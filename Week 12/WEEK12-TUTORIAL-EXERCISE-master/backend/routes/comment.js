...
const commentOwner = async (req, res, next) => {
    const [[comment]] = await pool.query('SELECT * FROM commnets WHERE id=?', [req.params.id])
  
    if (req.user.role === 'admin') {
      return next()
    }
  
    if (comment.commnet_by_id !== req.user.id) {
...
router.post('/:blogId/comments', isLoggedIn, async function (req, res, next) {
    ...
    try {
        const [rows1, fields1] = await conn.query(
            'INSERT INTO `comments` (`blog_id`, `comment`, `like`,`comment_by_id`, `comment_date`)'+
